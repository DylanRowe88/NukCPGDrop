...
static esp_err_t wildcard_handler(httpd_req_t *req)
{
    ...
    for (size_t i = 0; i < web_assets_count; i++) {
        if (strcmp(lookup, web_assets[i].path) == 0) {
            ...
            if (web_assets[i].compressed_len && web_assets[i].compressed_len < web_assets[i].len)
                httpd_resp_set_hdr(req, "Content-Encoding", "gzip");
            httpd_resp_set_hdr(req, "Cache-Control", "public, max-age=3600");
            size_t send_len = web_assets[i].compressed_len ? web_assets[i].compressed_len : web_assets[i].len;
...
esp_err_t web_server_start(void)
{
    ...
    ESP_LOGI(TAG, "HTTP server running on :80 (%u assets embedded)", web_assets_count);
    return ESP_OK;
}
