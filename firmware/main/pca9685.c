...
{
    esp_err_t ret = i2c_master_transmit(dev->dev_handle, &reg, 1, pdMS_TO_TICKS(10));
...
esp_err_t pca9685_set_pwm_freq(pca9685_t *dev, uint16_t freq_hz)
{
    ...
    uint8_t mode1;
    ESP_RETURN_ON_ERROR(pca9685_read_reg(dev, PCA9685_MODE1, &mode1), TAG, "read mode1");

    uint8_t sleep = mode1 | (1 << PCA9685_SLEEP_BIT);
    ESP_RETURN_ON_ERROR(pca9685_write_reg(dev, PCA9685_MODE1, sleep), TAG, "enter sleep");
    ESP_RETURN_ON_ERROR(pca9685_write_reg(dev, PCA9685_PRESCALE, prescale), TAG, "set prescale");
    ESP_RETURN_ON_ERROR(pca9685_write_reg(dev, PCA9685_MODE1, mode1), TAG, "restore mode1");

    vTaskDelay(pdMS_TO_TICKS(5));
    uint8_t restart = mode1 | (1 << PCA9685_AI_BIT);
    ESP_RETURN_ON_ERROR(pca9685_write_reg(dev, PCA9685_MODE1, restart), TAG, "restart");
    return ESP_OK;
}
...
esp_err_t pca9685_set_channel(pca9685_t *dev, uint8_t channel, uint16_t on, uint16_t off)
{
    ...
    uint8_t buf[5] = { reg, on & 0xFF, (on >> 8) & 0xFF, off & 0xFF, (off >> 8) & 0xFF };
    esp_err_t ret = i2c_master_transmit(dev->dev_handle, buf, 5, pdMS_TO_TICKS(10));
    if (ret == ESP_OK) dev->channel_cache[channel] = off;
    return ret;
}
                              ...
                              const uint16_t *values, uint8_t count)
{
    ...
    return i2c_master_transmit(dev->dev_handle, buf, 2 + count * 2, pdMS_TO_TICKS(10));
}
