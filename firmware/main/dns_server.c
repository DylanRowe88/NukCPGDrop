...
static void dns_reply_to_esp_ip(uint8_t *buffer, size_t len)
{
    ...
    memcpy(response + rlen, &p, 2); rlen += 2;
    response[rlen++] = 0x00; response[rlen++] = 0x01; // A record
    response[rlen++] = 0x00; response[rlen++] = 0x01; // IN class
    response[rlen++] = 0x00; response[rlen++] = 0x00; response[rlen++] = 0x00; response[rlen++] = 0x3C; // TTL 60
    response[rlen++] = 0x00; response[rlen++] = 0x04; // data len = 4
    response[rlen++] = 192; response[rlen++] = 168; response[rlen++] = 4; response[rlen++] = 1;
