...
static void action_btn_cb(lv_event_t *e) {
  ...
  bool all_held = true;
  for (int i = 0; i < SERVO_COUNT; i++) { if (!servos_is_held(i)) { all_held = false; break; } }
  if (all_held) servos_release_all();
...
static void diff_btn_cb(lv_event_t *e) {
  difficulty_t diff = (difficulty_t)(intptr_t)lv_obj_get_user_data(lv_event_get_target(e));
...
static void servo_dir_cb(lv_event_t *e) {
  ...
  if (idx >= 0 && idx < 6) {
    ...
    lv_obj_add_state(btn, g_state.servo_dir[idx] ? LV_STATE_CHECKED : LV_STATE_DEFAULT);
    state_save();
  }
}
...
static void create_difficulty_selector(lv_obj_t *parent) {
  ...
  lv_obj_t *l1 = lv_label_create(diff_btn_long); lv_label_set_text(l1, "Long"); lv_obj_center(l1);
  ...
  lv_obj_t *l2 = lv_label_create(diff_btn_short); lv_label_set_text(l2, "Short"); lv_obj_center(l2);
  ...
  lv_obj_t *l3 = lv_label_create(diff_btn_random); lv_label_set_text(l3, "Random"); lv_obj_center(l3);

  bool is_random = (g_state.difficulty == DIFFICULTY_RANDOM);
  int ry = y + 18 + bh + 8;

  range_min_slider = lv_slider_create(parent);
  lv_obj_set_size(range_min_slider, 100, 14); lv_obj_set_pos(range_min_slider, 50, ry);
  ...
  lv_obj_set_size(range_max_slider, 100, 14); lv_obj_set_pos(range_max_slider, 50, ry + 20);
...
static void create_system_status(lv_obj_t *parent) {
  int y = 48 + 2 * 64 + 8 + 18 + 40 + 12 + 18 + 28 + 16 + 18 + 20 + 14 + 14 + 22 + 18 + 34 + 8 + 20 + 20 + 20;
  ...
  for (int i = 0; i < 5; i++) {
    ...
    lv_obj_set_style_bg_color(bar_seg, lv_color_hex(0x333333), LV_STATE_DEFAULT);
...
static void create_servo_direction_group(lv_obj_t *parent) {
  int y = 48 + 2 * 64 + 8 + 18 + 40 + 12 + 18 + 28 + 16 + 18 + 20 + 14 + 14 + 22 + 18 + 34 + 8 + 20 + 20 + 20 + 18 + 34 + 28;
  ...
  for (int i = 0; i < SERVO_COUNT; i++) {
    ...
    lv_obj_set_user_data(btn, (void *)(intptr_t)i);
    lv_obj_add_state(btn, g_state.servo_dir[i] ? LV_STATE_CHECKED : LV_STATE_DEFAULT);
    ...
    lv_obj_t *lbl = lv_label_create(btn);
    char buf[16];
    lv_snprintf(buf, sizeof(buf), "%d %s", i + 1, g_state.servo_dir[i] ? "HI" : "LO");
...
static void create_sound_toggle(lv_obj_t *parent) {
  int y = 48 + 2 * 64 + 8 + 18 + 40 + 12 + 18 + 28 + 16 + 18 + 20 + 14 + 14 + 22 + 18 + 34 + 8 + 20 + 20 + 20 + 18 + 34 + 28 + 18 + 64 + 16;
...
static void create_audio_level(lv_obj_t *parent) {
  int y = 48 + 2 * 64 + 8 + 18 + 40 + 12 + 18 + 28 + 16 + 18 + 20 + 14 + 14 + 22 + 18 + 34 + 8 + 20 + 20 + 20 + 18 + 34 + 28 + 18 + 64 + 16 + 18 + 24 + 16;
...
void screen_main_update_progress(void) {
  ...
  if (completed > (uint32_t)SERVO_COUNT) completed = SERVO_COUNT;
  if (g_state.double_drop) completed = (completed + 1) / 2;
...
void screen_main_update_status(void) {
  ...
  for (int i = 0; i < SERVO_COUNT; i++) {
    if (servo_dir_btn[i]) {
      lv_obj_add_state(servo_dir_btn[i], g_state.servo_dir[i] ? LV_STATE_CHECKED : LV_STATE_DEFAULT);
      lv_obj_t *lbl = lv_obj_get_child(servo_dir_btn[i], 0);
      if (lbl) {
        char b[16];
        lv_snprintf(b, sizeof(b), "%d %s", i + 1, g_state.servo_dir[i] ? "HI" : "LO");
...
void screen_main_set_drop_anim(int idx) {
  ...
  if (idx >= 0 && idx < SERVO_COUNT && can_indicators[idx]) {
    lv_obj_set_style_bg_color(can_indicators[idx], lv_color_hex(0xffc107), LV_STATE_DEFAULT);
